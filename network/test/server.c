...
void* acceptt(){
	...
	while(1){
		...
		if(head!=tail) {
			...
			pthread_create(t+i,NULL,child,&tem);++i;
...
pthread_t t;	pthread_create(&t,NULL,acceptt,NULL);
	...
	switch (video){
		...
		while(1){
				...
				for(int z=0;z<tcwnd;z++){
					...
					pack->head.seq=ser_seq++;
					pack->head.ACK=ser_ack++;
					if(pack->head.seq > 46) {pack->head.seq=46;ser_seq = 47;}
					if(times>=46){
						pack->head.finish=1;
					}
					else
					...
					printf("cwnd = %d\n",cwnd);
					int loss = (rand()%100);
					if(isack[ser_seq-1] && ser_seq != 47) {z--;continue;}
					if(loss == 5) printf("lose %d \n",ser_seq-1);
					if(loss != 5){
						if ((n=sendto(fd, pack, sizeof(segment), 0, (struct sockaddr*)&client, length)) < 0) perror("Could not send datagram!!\n");
						else
		...
		while(1){
				...
				for(int z=0;z<tcwnd;z++){
					...
					pack->head.seq=ser_seq++;
					pack->head.ACK=ser_ack++;
					if(pack->head.seq > 258) {pack->head.seq=258;ser_seq=259;}
					if(times>=258){
						pack->head.finish=1;
					}
					else
					...
					printf("cwnd = %d\n",cwnd);
					int loss = (rand()%20);
					if(isack[ser_seq-1] && ser_seq !=259) {z--;continue;}
					if(loss == 5) printf("lose %d \n",ser_seq-1);
					if(loss != 5){
						if ((n=sendto(fd, pack, sizeof(segment), 0, (struct sockaddr*)&client, length)) < 0) perror("Could not send datagram!!\n");
						else
		...
		while(1){
				...
				for(int z=0;z<tcwnd;z++){
					...
					pack->head.seq=ser_seq++;
					pack->head.ACK=ser_ack++;
					if(pack->head.seq > 205) {pack->head.seq=205;ser_seq=206;}
					if(times>=205){
						pack->head.finish=1;
					}
					else
					...
					printf("cwnd = %d\n",cwnd);
					int loss = (rand()%20);
					if(isack[ser_seq-1] && ser_seq!=206) {z--;continue;}
					if(loss == 5) printf("lose %d \n",ser_seq-1);
					if(loss != 5){
						if ((n=sendto(fd, pack, sizeof(segment), 0, (struct sockaddr*)&client, length)) < 0) perror("Could not send datagram!!\n");
						else
